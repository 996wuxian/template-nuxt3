<template>
  <div class="music-page theme-page">
    <div class="container">
      <div>版块正在思索中...</div>
...
interface AnimateNotesReturn {
  notesG1: HTMLElement[]
  notesG2: HTMLElement[]
  notesG3: HTMLElement[]
  animateNotes: (els: HTMLElement[]) => void
}

const createNoteAnimation = (colors: string[]): AnimateNotesReturn => {
  ...
  return { notesG1, notesG2, notesG3, animateNotes }
}
...
onMounted(() => {
  ...
  // 获取颜色
  const { green, pink, blue, orange, cyan } = getCSSVariables()
  const colors = [green, pink, blue, orange, cyan, '#a3a4ec', '#67b5c0', '#fd7c6e']
  ...
  // 创建音符动画
  const { notesG1, notesG2, notesG3, animateNotes } = createNoteAnimation(colors)
...
.music-page {
  @apply flex p-10px b-rd-8px  pb-20px mt-10px h-340px ml-10px justify-center items-center;
...
.container {
  ...
  --orange: #ffcc81;
  --cyan: #7ed1e2;
  @apply w-100% h-100% flex flex-col justify-center items-center gap-20px color-#B2C2EC text-14px;

  svg {
    @apply w-100% overflow-visible;
  }
}
