<template>
  <div ref="containerRef" class="fs-book-waterfall-container" @scroll="handleScroll">
    <div ref="listRef" class="fs-book-waterfall-list">
      <div
      ...
      >
        <slot
          :image-height="state.cardPos[index].imageHeight"
          name="item"
          :item="item"
          :index="index"
        ></slot>
...
const computedImageHeight = (list: ICardItem[]) => {
  list.forEach(item => {
    ...
    console.log('🚀 ~ computedImageHeight ~ imageHeight:', imageHeight)
    state.cardPos.push({
